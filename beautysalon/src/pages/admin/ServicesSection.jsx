// src/pages/admin/ServicesSection.jsx
...
export default function ServicesSection() {
    ...
    // Modal state for delete/disable
    const [delModal, setDelModal] = useState({ open: false, id: null, name: '', refs: 0 });
    ...
    async function add(e) {
        ...
        if (error) alert(error.message); else { setForm({ name: '', duration_min: 60, price_cents: 0, category_id: filterCat || '', is_active: true }); load(); }
    }
    ...
    // OPEN STYLISH MODAL (instead of window.confirm)
    async function openDelete(id) {
        ...
        setDelModal({ open: true, id, name: svc?.name || 'this service', refs: count || 0 });
    }

    async function confirmDisable() {
        const { id } = delModal;
        const { error } = await supabase.from('services').update({ is_active: false }).eq('id', id);
        setDelModal({ open: false, id: null, name: '', refs: 0 });
        if (error) alert(error.message); else load();
    }
    ...
    return (
        <section>
            ...
            <ConfirmModal
                open={delModal.open}
                onClose={() => setDelModal({ open: false, id: null, name: '', refs: 0 })}
                onConfirm={delModal.refs > 0 ? confirmDisable : confirmDelete}
                title={delModal.refs > 0 ? "Disable service?" : "Delete service?"}
                confirmText={delModal.refs > 0 ? "Disable Service" : "Delete"}
                destructive={delModal.refs === 0}
            >
                {delModal.refs > 0 ? (
                    <p>
                        <span className="font-medium">{delModal.name}</span> is used in <b>{delModal.refs}</b> past appointment item(s).
                        Deleting would break history. We’ll <b>disable</b> it so it won’t be bookable, but history stays intact.
                    </p>
                ) : (
                    <p>
                        You’re about to permanently delete <span className="font-medium">{delModal.name}</span>. This action cannot be undone.
                    </p>
                )}
            </ConfirmModal>
        </section>
    ...
    return (
        <tr className="border-b">
            ...
            <td className="py-2 pr-3 text-right whitespace-nowrap">
                <button type="button" className="btn mr-2" onClick={() => onSave(e)}>Save</button>
